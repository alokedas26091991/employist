...
app.controller('cartCrt', ['$scope', '$http','$timeout','$log', '$sce', function ($scope, $http,$timeout) {
		...
		$scope.getTotal = function(){
			...
			for(var i = 0; i < $scope.cartitems.length; i++){
				...
				console.log(product.CartItems.item_gross_amount +"-"+ product.CartItems.discount_amt+"-"+product.CartItems.delivery_charge);
		...
		/*total Delivery Charge */
			...
			for(var i = 0; i < $scope.cartitems.length; i++){
				...
				total += parseFloat(product.CartItems.delivery_charge);
			}
		...
		//* total sub total */
			...
			for(var i = 0; i < $scope.cartitems.length; i++){
				var product = $scope.cartitems[i];
				console.log(eval($scope.cartitems[i].CartItems.item_gross_amount))
				total += parseFloat(product.CartItems.item_gross_amount);
			}
        ...
        $http.post(productLink + '/deletecartItem', {'info_id': info_id},{
            headers: {
		...
		$scope.updateQuentity=function($event,position,type){
			$event.preventDefault();
				...
				var total_quantity=$scope.cartitems[position]['Products'].total_quantity;
				...
				if(type==1){
					...
					if(quentity==0){
						...
						else if(minimum_order>quentity)
						{
						    ...
						    alert("You will have to buy the minimum order");
						    $scope.loaded = true;
						}
					    ...
					    if(quentity>=total_quantity)
						{
						    //$scope.ErrorMassage("Product Stock is not Sufficient");
						    alert("Product Stock is not Sufficient");
						    $scope.loaded = true;
						}
						else
						{
							quentity=eval(quentity)+1;
							updateQuantityData(quentity,info_id);
		...
		$scope.getDelivaryData = function(){
			...
			$scope.setDelivaryadd=function(id){
		...
		function updateQuantityData(quentity,infoid){
				$http.post(productLink + '/updateItem', {'info_id': infoid,'quantity':quentity},{
            headers: {
                'X-CSRF-Token': csrf_token
            }}).then(function (resp) {
            //$scope.loadingStatus = false;
		 ...
		 $scope.applyCoupon = function () {
            ...
            $http.post(productLink + '/checkcouponcode', {'coupon_code': $scope.coupon.data},{
            headers: {
                'X-CSRF-Token': csrf_token
            }}).then(function (resp) {
                if (resp.data.success == 1) {
    ...
    for(var i = 0; i < $scope.cartitems.length; i++){
        ...
        total1 += product1.CartItems.discount_amt;
    }
    return total1;
}
...
angular.bootstrap(document.getElementById("app5"), ['cart']); 
