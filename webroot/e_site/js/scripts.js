(function($) {
    ...
    function main() {
        ...
        $(".form-checkout .next").on('click', function() {
			...
			hiddenFrm=current_fs.find('input[type="hidden"]').val();
			console.log(current_fs.find('input[type="hidden"]').val());
			alert(hiddenFrm);
			return;
			if(hiddenFrm==1){
							...
							$.ajax({
								...
								if(html=='true'){
									next_fs = current_fs.closest('fieldset').next();
									//alert(JSON.stringify(next_fs));
								...
								$("#add_err").html("<i class='fa fa-spinner fa-spin' aria-hidden='true'></i> Loading...")
								}
							});
    ...
    function Learning() {
		...
		//alert($('.top-nav-list').children('li').hasClass('active'));
        if ($('.top-nav-list').children('li').hasClass('active')) {
            $('.learning-section')
                .toggleClass('learning-section-fly')
                .css('paddingRight', width);
        }
        $('.top-nav-list').find('.outline-learn, .discussion-learn, .note-learn, .batch-section').on('click', '> a', function(e) {
    ...
    $(document).ready(function() {
        ...
        $('.alert-white-alt').delay(5000).fadeOut('slow');
    });
